...
const MediaEditor: React.FC = () => {
  ...
  const [optionsPanelOpen, setOptionsPanelOpen] = useState(!isMobile);
  ...
  const toggleOptionsPanel = () => setOptionsPanelOpen(prev => !prev);
  
  useEffect(() => {
    setOptionsPanelOpen(!isMobile);
  }, [isMobile]);
      ...
      if (isVideo) {
        ...
        tempVideo.onloadedmetadata = () => {
          ...
          const newMedia = {
            ...
            height: 400,
            startTime: 0,
            endTime: Math.min(videoDuration, 5),
    ...
    <div className="h-screen flex flex-col overflow-hidden bg-gray-50">
      <header className="h-14 flex items-center justify-between px-4 border-b bg-white z-10">
        ...
        <div className="flex items-center space-x-2">
          ...
          <Button variant="default" size="sm" className="ml-2 bg-veed-orange hover:bg-orange-600 flex items-center">
            {!isMobile && 'Upgrade'} <Zap className={isMobile ? "" : "ml-1"} size={16} />
          </Button>
          
          <Button variant="default" size="sm" className="bg-veed-gray-900 hover:bg-veed-gray-800 flex items-center">
            {!isMobile && 'Done'} <Check className={isMobile ? "" : "ml-1"} size={16} />
          </Button>
        </div>
      </header>
      ...
      <div className="flex flex-1 overflow-hidden">
        {(!isMobile || sidebarOpen) && (
          <div className={`${isMobile ? 'absolute inset-0 z-40 bg-white' : 'w-16'} bg-white border-r flex flex-col items-center py-4 ${isMobile ? 'w-64' : ''}`}>
            {isMobile && (
        ...
        {optionsPanelOpen && (
          <div className={`${isMobile ? 'absolute inset-0 z-30 bg-white' : 'w-80'} bg-white border-r overflow-y-auto ${isMobile ? 'w-full' : ''}`}>
            {isMobile && (
              <div className="sticky top-0 w-full flex justify-between items-center px-4 py-3 bg-white border-b">
                <h2 className="font-medium">Media Options</h2>
                <button 
                  className="p-1.5 hover:bg-gray-100 rounded-md"
        ...
        <div className="flex-1 flex flex-col overflow-hidden relative">
          ...
          <div className={`${isMobile ? 'h-44' : 'h-56'} border-t bg-white`}>
            <div className="flex items-center justify-between p-2 border-b flex-wrap">
              {!isMobile && (
                <div className="flex items-center space-x-2">
